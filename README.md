...
An automated 60 FPS retro stickman fighting simulation and headless video generation pipeline designed for **YouTube Shorts**, **TikTok**, and **Facebook Reels**.

---

## ⚡ Core Features

- **1080x1920 9:16 Canvas**: Native portrait vertical video format for short-form social video feeds.
...
- **Procedural Web Audio Engine**: Zero-asset procedural 8-bit sound generator: high-impact crunches, metallic block clangs, announcer speech chirps, and driving cyberpunk synth battle music.
...
- **Headless Chromium Recorder**: Puppeteer launches headless browser, records canvas + synthesized audio into high-bitrate WebM/MP4.
...
- **GitHub Actions Automation**: Runs on a cron schedule 3x daily (`08:00`, `14:00`, `20:00` UTC) to publish fresh daily combat videos.
...
To run a headless simulation and export a 1080x1920 video to `output/`:
...
The workflow in `.github/workflows/generate_reel.yml` automatically triggers **2x daily** (at `08:00 UTC` and `20:00 UTC`), simulates a 3-round tournament, converts the footage to **9:16 vertical MP4 (1080x1920)** with Halal Duff audio, and sends it directly to your Telegram chat.
...
2. Select **Automated Stickman 9:16 Reel Pipeline** on the left menu.
...
5. Click the green **Run workflow** button. Within ~2-3 minutes, the 9:16 vertical MP4 match will be delivered straight to your Telegram!

### 4. Pushing to GitHub
```bash
git init
git add .
git commit -m "feat: 9:16 vertical FB reels MP4 pipeline with 2x daily telegram dispatch"
git branch -M main
